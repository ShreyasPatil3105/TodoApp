...
async def read_todo(user:user_dep,db:db_dep,todo_id:int=Path(gt=0)):
    if user is None:
        raise HTTPException(status_code=401,detail="Authentication Failed")
...
async def create_todo(user:user_dep,db:db_dep,todo_req:TodoRequest):
    if user is None:
        raise HTTPException(status_code=401,detail="Authentication Failed")
...
async def update_todo(user:user_dep,db:db_dep,todo_req:TodoRequest,todo_id:int=Path(gt=0)):
    if user is None:
        raise HTTPException(status_code=401,detail="Authentication Failed")
    todo_model=db.query(Todos).filter(Todos.id==todo_id)\
        .filter(Todos.owner_id==user.get('id')).first()
